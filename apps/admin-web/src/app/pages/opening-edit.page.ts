...
@Component({
  template: OPENING_EDIT_TEMPLATE,
})
export class OpeningEditPage extends EntityEditBasePage {
  @ViewChild('xiangqiBoard') private xiangqiBoard?: XiangqiBoardComponent;

  readonly boardComponentId = `opening-edit-board-${Math.random().toString(36).slice(2)}`;
  selectedMoveIndex = -1;
  ...
  constructor(route: ActivatedRoute, router: Router, api: AdminContentService) {
    super(route, router, api, 'openings');
  ...
  removeSelectedMove(): void {
    ...
    const moveTokens = this.currentMoveTokens();
  ...
  movePiece(event: XiangqiMove): void {
    ...
    const moveList = this.currentMoveTokens();
  ...
  private boardMoves(): XiangqiMove[] {
    return XiangqBoardUtils.parseMoveNotationList(this.form.controls['move_list']?.value);
  }
